...
class PaintingFinder:
    ...
    def getRankedListOfSimilars(self, inputImages, paintingsDB, method=1, showOutput=True):
        results = []
        for inputImage in inputImages:
            if method == 0:
                results.append(self.useResNet18Approach(inputImage, paintingsDB, showOutput))
            elif method == 1:
                results.append(self.useORBApproach(inputImage, paintingsDB, showOutput))
            elif method == 2:
                results.append(self.useAKAZEApproach(inputImage, paintingsDB, showOutput))
        return results
    ...
    def useORBApproachSingleImage(self, inputImage, secondImage):
        ...
        good = []
        nn_match_ratio = 0.75  # Nearest neighbor matching ratio
        for m, n in matches:
            if m.distance < nn_match_ratio * n.distance:
                good.append([m])
    ...
    def useAKAZEApproach(self, inputImage, paintingDB, showOutput):
        # Load the image convert to RGB and get gray version
        #inputImage = cv2.imread(inputImageFilename)
        inputImgRGB = cv2.cvtColor(inputImage, cv2.COLOR_BGR2RGB)
        inputImgGray = cv2.cvtColor(inputImgRGB, cv2.COLOR_RGB2GRAY)
        ...
        for imagefile, painting in paintingDB.items():
            ...
            #     if m.distance < nn_match_ratio * n.distance:
            #         good.append([m])
            good = self.findGoodMatchsUsingBFMatcherWithKnn(inputImageDescriptor, tmpImageDescriptor, bf)
    ...
    def findGoodMatchsUsingBFMatcherWithKnn(self, firstImageDescriptor, secondImagedescriptor, brutForcematcher, match_ratio=0.75, k=2):

        # Perform the matching between the ORB descriptors of the input image and the testing image

        matches = brutForcematcher.knnMatch(firstImageDescriptor, secondImagedescriptor, k)
        good = []
        # The matches with shorter distance are the ones we want.
    ...
    def useORBApproach(self, inputImage, paintingDB, showOutput):
        # Load the image convert to RGB and get gray version
        #inputImage = cv2.imread(inputImageFilename)
        inputImgRGB = cv2.cvtColor(inputImage, cv2.COLOR_BGR2RGB)
        inputImgGray = cv2.cvtColor(inputImgRGB, cv2.COLOR_RGB2GRAY)
    ...
    def useResNet18Approach(self, inputImage, paintingDB, showOutput):
