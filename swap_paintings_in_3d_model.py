...
def unrectify(inputFrame, mask, similarImage, originalImagePointsForProspectiveTransformation, is_rect, vp_rectified, vp_originalImagePointsForProspectiveTransformations, vps_failed):
    ...
    if not vps_failed:
        p21 = vp_originalImagePointsForProspectiveTransformations
...
def swap_paintings_3d_model():
    ...
    for imagefile in os.listdir(d_model_schreeshots_images_path):
        ...
        outim, triple_paintings_boxes_masks = detect_and_segment_paintings(image, False)
        unrectified = None
        c = 0
        for tpbm in triple_paintings_boxes_masks:
            ...
            # key = cv2.waitKey(0)
            rectified, originalImagePointsForProspectiveTransformations, is_rect, vp_rectified, vp_originalImagePointsForProspectiveTransformations, vps_failed = rectify(image, mask)

            # cv2.imshow('Rectified', rectified)
            # key = cv2.waitKey(0)

            similars = pf.getRankedListOfSimilars([vp_rectified], paintingsDB, 1, False)
            if similars is not None and len(similars) > 0:
                ...
                if unrectified is None:
                    unrectified = unrectify(image, mask, msImage, originalImagePointsForProspectiveTransformations, is_rect, vp_rectified, vp_originalImagePointsForProspectiveTransformations, vps_failed)
                else:
                    unrectified = unrectify(unrectified, mask, msImage, originalImagePointsForProspectiveTransformations, is_rect, vp_rectified, vp_originalImagePointsForProspectiveTransformations, vps_failed)
